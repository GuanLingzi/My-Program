...
{
	system("color 8F");
	int result = MessageBox(MB_APPLMODAL,"请问是否要启动该程序？若启动，会占用较大cpu，此程序无毒！","system",MB_ICONASTERISK|MB_YESNO);
    if (result == IDNO)
    {
    	return 0;
	}
	else 
	{
		...
		remove("eason2.bat");
		remove("easo.txt");
	    result = MessageBox(MB_APPLMODAL,"程序已结束！","system",MB_ICONASTERISK|MB_OK);
		return 0;
    }
